...
{
    // Свойства DTO для хранения данных о продукте в корзине
    ...
    // Метод для маппинга сущности ProductsEntity на DTO ProductsShoppinDTO
    public void Mapping(Profile profile)
    {

        profile.CreateMap<ProductsEntity, ProductsShoppinDTO>()
            // Указание соответствия свойств сущности и DTO
            ...
            // Вычисление количества продукта в корзине
            .ForMember(dest => dest.QuantyProductCart, opt => opt.MapFrom(src => src.ShoppingCarts.Sum(s => s.QuantyProduct)))
            // Вычисление суммы продукта в корзине (цена * количество)
            .ForMember(dest => dest.SumProductCart, opt => opt.MapFrom(src => src.Price * src.ShoppingCarts.Sum(s => s.QuantyProduct)))
